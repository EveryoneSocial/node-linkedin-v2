...
module.exports = class LinkedInRestClient {
   ...
   * @returns {Promise<Object>}
   */
  async getTotalConnectionsNumber(access_token, id) {
    if (!access_token) throw new Error('Access code cannot be empty');
    const urnPath = encodeURIComponent(`urn:li:person:${id}`)
    const url = `${this.url}/connections/${urnPath}`
    return this.invoke('GET', url, undefined, undefined, { withAuth: true, access_token });
  }
  ...
  async richMediaShare(media_url, access_token, filename, id) {
    // Register for asset urn
    const response = await this.registerUpload(access_token, id)
    const asset_urn = response.value.asset
    const upload_url = response.value.uploadMechanism["com.linkedin.digitalmedia.uploading.MediaUploadHttpRequest"].uploadUrl
    // Upload the asset
    await this.postToAssetsApi(media_url, upload_url, filename, access_token)
    return asset_urn
  }

  async registerUpload(access_token, id){
    if (!access_token) throw new Error('Access code cannot be empty')
    ...
    return this.invoke(
      ...
      { withAuth: true, access_token }
    )
  }

  async postToAssetsApi(media_url, url, filename, access_token) {
    if (!access_token) throw new Error('Access code cannot be empty')
    const self = this;
    request(media_url, {encoding: 'binary'}, function(error, response, body) {
  ...
  async postMediaFromAccount(media_url, filename, access_token) {
    if (!access_token) throw new Error('Access code cannot be empty')
    const url = 'https://api.linkedin.com/media/upload'
    const image = await request(media_url)
  ...
  /**
   * Retrieve socialActions by urn
   *
   * @param urn
   * @param access_token
   * @returns {Promise<Object>}
   */
  async getSocialActionsSummary(urn, access_token) {
    if (!urn) throw new Error('Share urn can not be empty')
    if (!access_token) throw new Error('Access code cannot be empty');
    const url = `${this.url}/socialActions/${encodeURIComponent(urn)}`
    return this.invoke('GET', url, undefined, undefined, { withAuth: true, access_token });
  }
  ...
  invoke(method, url, headers = { 'Content-Type': 'Application/json' }, body = {}, auth = { withAuth: false, access_token: null }) {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        console.log(`LINKEDIN-OPTIONS-ERROR........ ${JSON.stringify(err)}`)
        return reject(err);
      }
      request(options, (error, response, body) => {
        console.log(`LINKEDIN-ERROR........ ${JSON.stringify(error)}`)
        console.log(`LINKEDIN-RESPONSE........ ${JSON.stringify(body)}`)
        ...
        if (body) {
          try {
            ...
            console.log(`LINKEDIN-DATA........ ${JSON.stringify(data)}`)
            return resolve(data);
          } catch (err) {
            console.log(`LINKEDIN-ERROR........ ${JSON.stringify(err)}`)
            return reject(err)
          }
        } else {
          return resolve([])
  ...
  generateOptions(method, url, headers, body, auth) {
    ...
    } else if (body && body['fileUpload']) {
      options['body'] = body['fileUpload']
    } else if (body) {
