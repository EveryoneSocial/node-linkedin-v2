...
module.exports = class LinkedInRestClient {
  ...
  async postToAssetsApi(media_url, api_url, filename, access_token) {
    if (!access_token) throw new Error('Access code cannot be empty')
    var self = this;
    request(media_url, {encoding: 'binary'}, function(error, response, body) {
      const f = fs.writeFile(filename, body, 'binary', function (err) {});
      return self.invoke(
        'PUT',
        api_url,
  ...
  generateOptions(method, url, headers, body, auth) {
    ...
    if (body && body['formData']) {
      ...
      options['upload-file'] = body['fileUpload']
    } else if (body) {
